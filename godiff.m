        ...
        for p = 1:length(players)
            ...
            for p2 = 1:length(players)
                if (p2 ~= p)
                    ...
                    divSURF_dist = divSURF_dist + surf_dist(imgB, imgA);
            ...
            divSURF_dist = divSURF_dist / (length(players) - 1);
            divSURF_pointCount = divSURF_pointCount / (length(players) - 1);
            
            str = sprintf('%u,%u,%u,%u,%u,%u,%s,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%s,%s', ...
                           session, com, coo, choice, rand, ...
                           (p+1), round, divCorr2, ...
                           divSSIM, divSSIM_idx, divSSIM_L1, ...
                           divSURF_dist, divSURF_pointCount, ...
                           'NA', 'NA');
